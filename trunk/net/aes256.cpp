...
ByteArray::size_type Aes256::encrypt_start(const ByteArray::size_type plain_length, ByteArray& encrypted)
{
    ...
    if (m_remainingLength % AES_BLOCK_SIZE != 0)
        padding = (AES_BLOCK_SIZE - (m_remainingLength % AES_BLOCK_SIZE));
...
{
    if (m_buffer_pos == AES_BLOCK_SIZE)
    {
        encrypt(m_buffer);

        for (m_buffer_pos = 0; m_buffer_pos < AES_BLOCK_SIZE; ++m_buffer_pos)
        {
...
ByteArray::size_type Aes256::encrypt_end(ByteArray& encrypted)
{
    if (m_buffer_pos > 0)
    {
        while (m_buffer_pos < AES_BLOCK_SIZE)
...
void Aes256::check_and_decrypt_buffer(ByteArray& plain)
{
    ...
    else if (m_decryptInitialized && m_buffer_pos == AES_BLOCK_SIZE)
    {
        decrypt(m_buffer);
