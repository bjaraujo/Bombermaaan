...
int findResourceElement(int type, const char* name, int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if (strncmp(resources[type][i].r_name, name, strlen(name)) == 0 && strlen(resources[type][i].r_name) == strlen(name))
        {
            found = true;
            break;
        }
...
bool generateSourceCode(int hBitmapCount, int hSoundCount, int realBitmapCount, int realSoundCount)
{
    ...
    if (realBitmapCount <= 0 && realSoundCount <= 0)
        return false;
    ...
    for (int type = TYPE_BITMAP; type < NUM_TYPES && !error; type++)
    {
        ...
        for (int i = 0, j = 0; i < count; i++)
        {
            ...
            {
                cerr << "WARNING: " << resources[type][i].r_name << " (" << resources[type][i].r_number << ") will not be included. Empty path." << endl;
                continue;
            }
...
int main(int argc, char** argv)
{
    ...
    {
        fprintf(stderr,
            "WARNING: the number of bitmap resources (%d) in the "
        ...
        fprintf(stderr,
            "Incomplete resources (i.e. its ID in the header or "
    ...
    {
        fprintf(stderr,
            "WARNING: the number of sound resources (%d) in the "
    ...
    {
        fprintf(stderr,
            "WARNING: only %d out of %d bitmap resources were "
    ...
    {
        fprintf(stderr,
            "WARNING: only %d out of %d sound resources were "
            "correctly parsed from the resource header (" HFILE ").\n",
            soundNo, hSoundCount);
    }
