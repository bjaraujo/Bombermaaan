...
CDrawGame::~CDrawGame() = default;

// Before using a CDrawGame, you must create it.

void CDrawGame::Create()
{
    CModeScreen::Create();
...
void CDrawGame::Destroy() { CModeScreen::Destroy(); }

void CDrawGame::OpenInput() { m_pInput->GetMainInput().Open(); }

void CDrawGame::CloseInput() { m_pInput->GetMainInput().Close(); }

EGameMode CDrawGame::Update()
{
    ...
    else if (m_ModeTime <= DRAWGAME_BLACKSCREEN_DURATION + DRAWGAME_TITLE_DURATION && !m_HaveToExit)
    {
        ...
        // Animate the flag
        if (m_FlagTimer < ANIMFLAG_TIME_0)
            m_FlagSprite = FLAG_SPRITE0;
        else if (m_FlagTimer < ANIMFLAG_TIME_1)
            m_FlagSprite = FLAG_SPRITE1;
        else
        {
            // Set next sprite
            m_FlagSprite = FLAG_SPRITE0;
        ...
        if (m_BigFumeTimer < ANIMBIGFUME_TIME_0)
            m_BigFumeSprite = BIGFUME_SPRITE0;
        else if (m_BigFumeTimer < ANIMBIGFUME_TIME_1)
            m_BigFumeSprite = BIGFUME_SPRITE1;
        else
        {
            // Set next sprite
            m_BigFumeSprite = BIGFUME_SPRITE0;
        ...
        if (m_SmallFumeTimer < ANIMSMALLFUME_TIME_0)
            m_SmallFumeSprite = SMALLFUME_SPRITE0;
        else if (m_SmallFumeTimer < ANIMSMALLFUME_TIME_1)
            m_SmallFumeSprite = SMALLFUME_SPRITE1;
        else
        {
            // Set next sprite
            m_SmallFumeSprite = SMALLFUME_SPRITE0;
