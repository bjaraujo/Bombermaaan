...
void CArenaCloser::Destroy() { Stop(); }

void CArenaCloser::Start()
{
...
void CArenaCloser::CreateSpiralClosing()
{
...
void CArenaCloser::CreateHorizontalClosing()
{
    SClosePosition ClosePosition;
    int I = 0;
    int X = -1;
...
void CArenaCloser::CreateVerticalClosing()
{
    SClosePosition ClosePosition;
    int I = 0;
    int X;
    ...
    if (m_IsClosing)
    {
        ...
        if (!m_ClosureData.empty())
        {
            ...
            if (m_TimeLeftBeforeClosingNextBlock <= 0.0f)
            {
                ...
                {
                    if (m_pArena->GetWall(Index).Exist() && m_pArena->GetWall(Index).GetType() == WALL_HARD
                        && m_pArena->GetWall(Index).GetBlockX() == BlockX && m_pArena->GetWall(Index).GetBlockY() == BlockY)
                    {
