...
void CBoard::Update()
{
    ...
    if (m_AnimateClock)
    {
        ...
        if (m_pOptions->GetTimeStartMinutes() > 0 || m_pOptions->GetTimeStartSeconds() > 0)
        {
            ...
            if (m_pClock->GetMinutes() < m_pOptions->GetTimeUpMinutes() || (m_pClock->GetMinutes() == m_pOptions->GetTimeUpMinutes() && m_pClock->GetSeconds() <= m_pOptions->GetTimeUpSeconds()))
            {
                // Animate the bottom of the clock (fast)
                if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE0;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 2)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE1;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 3)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE2;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 4)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE3;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 5)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE4;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 6)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE5;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 7)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE6;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_FAST * 8)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE7;
                else
                {
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE0;
                    m_ClockBottomTimer = 0.0f;
                ...
                if (m_ClockTopTimer < ANIMCLOCKTOP_TIME)
                    m_ClockTopSprite = ANIMCLOCKTOP_SPRITE0;
                else if (m_ClockTopTimer < ANIMCLOCKTOP_TIME * 2)
                    m_ClockTopSprite = ANIMCLOCKTOP_SPRITE1;
                else
                {
                    m_ClockTopSprite = ANIMCLOCKTOP_SPRITE0;
                    m_ClockTopTimer = 0.0f;
            ...
            else
            {
                ...
                if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE0;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 2)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE1;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 3)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE2;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 4)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE3;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 5)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE4;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 6)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE5;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 7)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE6;
                else if (m_ClockBottomTimer < ANIMCLOCKBOTTOM_NORMAL * 8)
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE7;
                else
                {
                    m_ClockBottomSprite = ANIMCLOCKBOTTOM_SPRITE0;
...
void CBoard::Display()
{
    ...
    for (Player = 0; Player < MAX_PLAYERS; Player++)
    {
        ...
        if (m_pOptions->GetBomberType(Player) != BOMBERTYPE_OFF && m_pArena->GetBomber(Player).HasExisted())
        {
