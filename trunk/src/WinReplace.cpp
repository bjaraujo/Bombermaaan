...
static int file_select(const struct dirent* entry)
{
    ...
    else
    {
        ...
        if (fileSelectSuffix != NULL)
        {
            ...
            if (strcasecmp(entry->d_name + l - strlen(fileSelectSuffix), fileSelectSuffix) != 0)
                return false;
        }
        return true;
    }
...
long _findfirst(const char* pattern, _finddata_t* findData)
{
    ...
    { // backslash present, divide between directory and search pattern
...
long _findnext(long handle, _finddata_t* findData)
{
    ...
    while (d != NULL)
    {
        ...
        if (strcasecmp(d->d_name + fileNameLength - strlen(findData->suffix), findData->suffix) == 0)
        {
            // found
            if (findData->name != NULL)
            {
 ...
 */
LPVOID LockResource(HGLOBAL hGlobal) { return currentResource; }
 ...
 */
DWORD SizeofResource(HMODULE hModule, HRSRC hResource) { return currentResourceLen; }
