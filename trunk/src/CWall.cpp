...
void CWall::Destroy(void) { CElement::Destroy(); }
...
bool CWall::Update(float DeltaTime)
{
    ...
    else if (m_Type == WALL_SOFT)
    {
        ...
        if (m_Burning)
        {
            if (m_Timer < ANIMBURNING_TIME1)
                m_Sprite = ANIM_BURNING1;
            else if (m_Timer < ANIMBURNING_TIME2)
                m_Sprite = ANIM_BURNING2;
            else if (m_Timer < ANIMBURNING_TIME3)
                m_Sprite = ANIM_BURNING3;
            else if (m_Timer < ANIMBURNING_TIME4)
                m_Sprite = ANIM_BURNING4;
            else if (m_Timer < ANIMBURNING_TIME5)
                m_Sprite = ANIM_BURNING5;
            else
            {
                // Dead
                m_Dead = true;
    ...
    {
        // Animate
        if (m_Timer < ANIMFALLING_TIME1)
            m_Sprite = ANIM_FALLING1;
        else if (m_Timer < ANIMFALLING_TIME2)
            m_Sprite = ANIM_FALLING2;
        else if (m_Timer < ANIMFALLING_TIME3)
            m_Sprite = ANIM_FALLING3;
        else if (m_Timer < ANIMFALLING_TIME4)
            m_Sprite = ANIM_FALLING4;
        else
        {
