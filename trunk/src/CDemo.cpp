...
void CDemo::UpdateMatch(void)
{
    ...
    if (m_Options.GetTimeUpMinutes() != 0 || m_Options.GetTimeUpSeconds() != 0)
    {
        ...
        if (!m_NoticedTimeUp && !m_Arena.GetArenaCloser().IsClosing())
        {
            ...
            if (m_Clock.GetMinutes() < m_Options.GetTimeUpMinutes() || (m_Clock.GetMinutes() == m_Options.GetTimeUpMinutes() && m_Clock.GetSeconds() <= m_Options.GetTimeUpSeconds()))
            {
...
void CDemo::ManageMatchOver(void)
{
    ...
    else if (m_Options.GetTimeUpMinutes() == 0 && m_Options.GetTimeUpSeconds() == 0 && (m_Options.GetTimeStartMinutes() != 0 || m_Options.GetTimeStartSeconds() != 0) && m_Clock.GetMinutes() == 0 && m_Clock.GetSeconds() == 0)
    {
        // Match is over
        m_MatchOver = true;
