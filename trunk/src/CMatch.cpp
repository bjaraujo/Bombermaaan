                    ...
                    for (int Player = 0; Player < MAX_PLAYERS; Player++)
                    {
                        ...
                        if (m_pOptions->GetBomberType(Player) == BOMBERTYPE_NET)
                        {
                            ...
                            if (m_Arena.GetBomber(Player).IsAlive())
                            {
                                ...
                                {
                                    m_Arena.GetBomber(Player).Command(
                                        CommandChunk.GetStepMove(Step), CommandChunk.GetStepAction(Step));
...
void CMatch::ManageMatchOver()
{
    ...
    if (m_pPauseMessage == nullptr)
    {
        ...
        else if (CountTeamsAlive == 1 && CountTeamsDying == 0)
        {
            ...
            && (m_pOptions->GetTimeStartMinutes() != 0 || m_pOptions->GetTimeStartSeconds() != 0) && m_Clock.GetMinutes() == 0
            && m_Clock.GetSeconds() == 0)
        {
            // Match is over
            m_MatchOver = true;
