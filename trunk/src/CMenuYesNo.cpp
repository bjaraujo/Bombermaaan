        ...
        && CurrentGameMode != GAMEMODE_HELP)
    {
        ...
        if (m_pInput->GetMainInput().TestBreak())
        {
            ...
            if (!m_Active)
            {
                ...
                {
                case GAMEMODE_MENU:
                    strcpy(m_Title, "EXIT MENU");
                    break;
                case GAMEMODE_TITLE:
                    strcpy(m_Title, "EXIT GAME");
                    break;
                default:
                    strcpy(m_Title, "EXIT MATCH");
                    break;
                }
    ...
    if (m_Active)
    {
        ...
        else if (m_pInput->GetMainInput().TestNext())
        {
            ...
            if (m_YesNo)
            {
                ...
                {
                case GAMEMODE_MENU:
                    NextGameMode = GAMEMODE_TITLE;
                    break;
                case GAMEMODE_TITLE:
                    NextGameMode = GAMEMODE_EXIT;
                    break;
                default:
                    NextGameMode = GAMEMODE_TITLE;
                    break; // NextGameMode = GAMEMODE_MENU
                }
            }
